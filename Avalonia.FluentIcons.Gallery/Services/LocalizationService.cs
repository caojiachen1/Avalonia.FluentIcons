...
public static class LocalizationService
{
    ...
    private static Dictionary<string, Dictionary<string, string>> Build()
    {
        var raw = new Dictionary<string, (string Zh, string En)>
        {
            ...
            ["home.nuget_install"] = Z("方式一：通过本地 NuGet 包安装（推荐）", "Option 1: Install via Local NuGet Package (Recommended)"),
            ["home.source_code"] = Z("方式二：引用源代码", "Option 2: Project Reference (Source Code)"),
            ...
            ["tutorial.step1_title"] = Z("安装", "Installation"),
            ["tutorial.nuget_install"] = Z("方式一：通过本地 NuGet 包安装（推荐）", "Option 1: Install via Local NuGet Package (Recommended)"),
            ["tutorial.nuget_csproj"] = Z("方式二：在 .csproj 中添加 PackageReference", "Option 2: Add PackageReference in .csproj"),
            ["tutorial.source_code"] = Z("方式三：引用源代码", "Option 3: Project Reference (Source Code)"),
            ["tutorial.step1_note"] = Z("NuGet 包位于仓库根目录，需要 Avalonia 11.2.0 或更高版本。", "The .nupkg file is in the repository root. Requires Avalonia 11.2.0 or later."),
